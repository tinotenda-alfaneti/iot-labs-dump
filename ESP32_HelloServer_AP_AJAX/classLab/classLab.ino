...
void setup()
{
  ...
  server.begin();
  server.on("/", base);
  server.on("/view-ten",getLastTen);
  server.on("/change-mode",changeModefunct);
  server.on("/start-pump",startPumpfunct);
  // server.on("/get-data", getData);
  server.on("/temp-humi", tempFunct);
  Serial.begin(9600);
...
void tempFunct() {
  ...
  //only this is sent
  server.send(200, "text/plain", tempStr + "," + humiStr);
  Serial.println("temp-printed");
}
